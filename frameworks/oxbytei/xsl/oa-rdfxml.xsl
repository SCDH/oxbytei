    ...
    <xsl:template name="scdh-onTextSpan-text">
        ...
        <xsl:variable name="extracted">
            <xsl:apply-templates mode="extract"
                select="$context//*[@xml:id eq $startId]/(following::text() | following::*[empty(child::node())]) intersect ($context//*[@xml:id eq $endId])/(preceding::text() | preceding::*[empty(child::node())])"
            />
        </xsl:variable>
